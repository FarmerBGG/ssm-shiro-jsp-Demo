...
var saleSwitchAjax = function(appId,obj){
	alert(appId+"/"+obj.attr("status"));
	$.ajax({
		type:"GET",
		url:path+"/sale.json",
		data:{appid:appId,status:obj.attr("status")},
		dataType:"json",
		success:function(data){
			...
			alert("join in");
				if(data.resultMsg === "success"){//操作成功
					if("open" === obj.attr("saleSwitch")){//上架操作
						//alert("恭喜您，【"+obj.attr("appsoftwarename")+"】的【上架】操作成功");
						//bug:obj并没有变，只是html变了，所以要刷新页面（暂定此解决方案）
					...
					}else if("close" === obj.attr("saleSwitch")){//下架操作
		...
		error:function(data){
			...
			if("open" === obj.attr("saleSwitch")){
				alert("抱歉，【"+obj.attr("appsoftwarename")+"】的【上架】操作遇到异常");
			}else if("close" === obj.attr("saleSwitch")){
				alert("抱歉，【"+obj.attr("appsoftwarename")+"】的【下架】操作遇到异常");
			}
		}
	});
};
